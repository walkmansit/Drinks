...
class OrderDetailsAdapter() : RecyclerView.Adapter<OrderDetailsAdapter.OrderDetailViewHolder>() {
    ...
    fun setSource(source: List<OrderDetailFull>) {
        ...
        val diffCallback = object : DiffUtil.Callback() {
            override fun areItemsTheSame(oldPos: Int, newPos: Int): Boolean =
                items[oldPos].id == source[newPos].id

            override fun getOldListSize(): Int = items.size
            override fun getNewListSize(): Int = source.size
            override fun areContentsTheSame(oldPos: Int, newPos: Int): Boolean =
                items[oldPos].hashCode() == source[newPos].hashCode()
        }
    ...
    class OrderDetailViewHolder private constructor(val binding: ViewOrderDetailsItemBinding) :
        RecyclerView.ViewHolder(binding.root), ItemTouchViewHolder {
        override fun onItemSelected() {
            itemView.background =
                ContextCompat.getDrawable(itemView.context, R.drawable.bg_swipe_to_delete_selected)
        }

        override fun onItemCleared() {
            itemView.background =
                ContextCompat.getDrawable(itemView.context, R.drawable.bg_round_border_with_solid)
        }
